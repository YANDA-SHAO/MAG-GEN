...
parser.add_argument("--cam_r_min", type=float, default=1.3)
parser.add_argument("--cam_r_max", type=float, default=3.2)
...
def main():
    ...
    if FLAGS.view_cluster:
        anchor_rng = np.random.default_rng(FLAGS.seed + 99999)
        anchor_view = sample_anchor_view(anchor_rng)
        scene_jitter = sample_scene_jitter_ranges(anchor_rng)
    ...
    meta_scene = {
        ...
        "min_camera_baseline": float(FLAGS.min_camera_baseline),
        "max_camera_baseline": float(FLAGS.max_camera_baseline),
        "scene_jitter": None if scene_jitter is None else {
            "az_jitter_deg": float(scene_jitter["az_jitter_deg"]),
            "el_jitter_deg": float(scene_jitter["el_jitter_deg"]),
            "radius_jitter": float(scene_jitter["radius_jitter"]),
            "focal_jitter_mm": float(scene_jitter["focal_jitter_mm"]),
    ...
    for view_idx in range(FLAGS.num_views):
        ...
        if FLAGS.view_cluster:
            assert anchor_view is not None
            if view_idx == 0:
                az_deg, el_deg, radius, focal_mm = anchor_view
            else:
                az_deg, el_deg, radius, focal_mm = sample_view_params_near_anchor(
                    anchor_view[0], anchor_view[1], anchor_view[2], anchor_view[3],
                    center_for_views, view_rng, scene_jitter
                )
        else:
        ...
        world_to_camera, camera_to_world = compute_extrinsic_matrices(cam_pos, center_for_views)
        ...
        baseline_to_anchor = None
        if FLAGS.view_cluster and anchor_view is not None:
            anchor_pos = camera_position_from_spherical(center_for_views, anchor_view[0], anchor_view[1], anchor_view[2])
            baseline_to_anchor = float(np.linalg.norm(cam_pos - anchor_pos))
        ...
        meta_view = {
            ...
            "extrinsic_camera_to_world": camera_to_world.tolist(),
            "view_cluster": bool(FLAGS.view_cluster),
            "baseline_to_anchor": None if baseline_to_anchor is None else float(baseline_to_anchor),
