...
Generate synthetic motion magnification triplets (A, B, Y) using a physics-based
Kubric simulation.

The script builds random scenes with 3D objects and cameras, runs a short
PyBullet simulation, and renders three frames:
    A : reference frame
    B : small-motion frame
    Y : magnified-motion frame

The magnified frame is generated by applying a rigid motion delta scaled by
the magnification factor (alpha).
...
parser.add_argument("--focal_mm_min", type=float, default=20.0,
                    help="Min focal length in mm. Smaller -> wider FOV (objects less likely out-of-frame).")
parser.add_argument("--focal_mm_max", type=float, default=40.0,
...
parser.add_argument("--px_ab_min", type=float, default=0.005,
                    help="Target pixel displacement magnitude for A->B (tiny, almost invisible).")
parser.add_argument("--px_ab_max", type=float, default=0.1,
